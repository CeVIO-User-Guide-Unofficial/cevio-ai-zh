...
版本 8.5 实装了大量提升歌唱表现力的新调整滑块。另外还改进了歌唱·语音功能，使其更易于使用。※现在公开中的仅为手动升级安装包。
...
- 实装了 3 个新的调整滑块「Husky」「Tune」和「音高」。
    ![3 new sliders](images/V8.5_new3slider.png)
    - 「Husky」滑块向上推得越多，声音越沙哑。※不影响调整画面。
    - 「Tune」滑块向上推得越多，歌手将越忠实地按照乐谱的音程（即更机械地）演唱。※影响音高调整画面。
    - 「音高」滑块以 440Hz 为基准偏移导出的声音。※不影响调整画面。
...
- 在 TIMING 调整页面按住 ++alt++ 键可以暂时显示或隐藏状态线。
