...
| --- | --- |
| ++1++ | 编辑乐谱。 |
| ++2++ | 调整时间。++shift+2++ 叠加显示时间调整界面（TMG）。 |
| ++3++ | 调整音量。++shift+3++ 叠加显示音量调整界面（VOL）。 |
| ++4++ | 调整音高。++shift+4++ 叠加显示音高调整界面（PIT）。 |
| ++5++ | 调整颤音。++shift+5++ 叠加显示颤音调整界面（VIA）。 |
| ++6++ | 调整音色。++shift+6++ 叠加显示音色调整界面（ALP）。 |
| ++7++ | 调整沙哑度。++shift+7++ 叠加显示沙哑度调整界面（HUS）。 |
| ++ctrl+tab++ | 显示下一个调整界面。 |
...
| ++ctrl+i++ | 显示 / 隐藏时间调整界面的状态线。</br>※在隐藏状态线的情况下，调整时会自动保证状态线之间的原始比率。 |
